...
class Window(Tk):
	...
	def initialize(self):
		...
		imp.reload(sys)
		#sys.setdefaultencoding("utf-8")    # has always been discouraged,
...
#========================================================== end displaying item
		...
		self.wButton = Button(self, text='Test', command = lambda: self.test())
		self.wButton.pack(side = RIGHT)
		...
		self.wButton.pack(side = LEFT)
		self.wButton = Button(self, text='Remove', command =lambda: self.Remove(content,"[edit]",self.editNo.get("1.0", "end-1c"),self.mylist))
		self.wButton.pack(side = LEFT)
...
#========================================================== start test
	def Remove(self,content,addOrEdit,Num,mylist ):
		...
		Record.writeRecordToLog(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),"1","gg","Remove")
		display
	def test(self ):
		print("gg")
		#Record.writeRecordToLog("20150101","1","gg","add")
#========================================================== end test
	...
	def OnButtonClick(self,content,addOrEdit,Num,mylist ):
		...
		if (addOrEdit =="[edit]" and not Num.isdigit()):
			print("leave")
			var = messagebox.showinfo("Waring" , "Please give a correct input!")
			return
	...
	def addNewBook(self,content,mylist):
		...
		Record.writeRecordToLog(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),"1","gg","Add")
...
#============================================== end display
	...
	def editBook(self,content,Num,mylist):
		...
		Record.writeRecordToLog(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),"1","gg","Edit")
